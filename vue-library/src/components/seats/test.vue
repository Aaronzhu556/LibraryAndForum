<template>
    <div>
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>社区图书馆</el-breadcrumb-item>
            <el-breadcrumb-item>位置预约</el-breadcrumb-item>
        </el-breadcrumb>

        <el-card>
            <!--搜索与添加-->
            <el-select v-model="seat_value" placeholder="请选择楼层"  @change="currentSeatChange">
                <el-option
                        v-for="item in seat_options"
                        :key="item.seat_id"
    ...
    export default {
        ...
        methods:{
            ...
            getAllSeats(){
                axios.get('/api/seat/getallseat',{
                    ...
                    if (parseInt(response.data.code) === 200){
                        this.seat_options = response.data.object;
            ...
            buySeat: function(){
                    ...
                    for(let j=0;j<this.seatCol;j++){
                        if(oldArray[i][j]===1){
                            var item = {
                                seatinfo_row:i,
                                seatinfo_col:j,
                                seatinfo_parent:this.seat_value,
                            };
                            // item.seatinfo_row = i;
                            // item.seatinfo_col = j;
                            seatlist.push(item)
                ...
                axios.post('/api/seatinfo/addseatinfo',JSON.stringify({
                    seatInfo_list : seatlist
                }), {
                    headers: {
            ...
            initSeatArray(){
                ...
                for (var i=0;i<this.seat.seat_info.length;i++){
                    var row = this.seat.seat_info[i].seatinfo_row;
                    var col = this.seat.seat_info[i].seatinfo_col;
                    this.seatArray[row][col] = 2;
                }
             ...
             getSeatInfo(){
                axios.get('/api/seatinfo/searchallseatinfo',{
                    params:{
                        seat_id : this.seat_value,
                    },
                    headers:{Authorization: sessionStorage.getItem('token')}
                }).then((response) => {
                    if (parseInt(response.data.code) === 200){
                        this.seat = response.data.object;

                        this.seatRow = this.seat.seat_row;
                        this.seatCol = this.seat.seat_col;
                        this.initSeatArray();
